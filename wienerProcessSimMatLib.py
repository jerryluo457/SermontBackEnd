...
# 2. Setup Timeline
    ...
    wealth_paths = np.zeros((num_runs, total_steps + 1)) # <-- CHANGED: Use total_steps
...
# 5. Run the Simulation Loop (Vectorized across paths, sequential across time)
    for t in range(total_steps): # <-- CHANGED: Loop over all fine steps
        ...
        # A. Check Hazard Rate for all active paths (Scaled by dt)
        hazard = get_value_at_t(hazard_rates, year_idx) * dt # <-- CHANGED
        new_hazards = np.random.rand(num_runs) < hazard
        forced_retirement = np.logical_or(forced_retirement, new_hazards)
        
        # B. Apply Market Move (Euler-Maruyama approximation of GBM)
        ...
        # C. Add Contributions & Subtract Expenses (Scaled by dt)
        contribution = get_value_at_t(contrib_timeline, year_idx) * dt # <-- CHANGED
        
        working_mask = ~forced_retirement & (current_age_float < retirement_age)
        retired_mask = forced_retirement | (current_age_float >= retirement_age)
        
        wealth_paths[working_mask, t+1] += contribution
        wealth_paths[retired_mask, t+1] -= (wealth_paths[retired_mask, t+1] * (0.04 * dt)) # <-- CHANGED
            
        # D. Apply Debt Drag (Scaled by dt)
        wealth_paths[:, t+1] -= (annual_debt_drag * dt) # <-- CHANGED
